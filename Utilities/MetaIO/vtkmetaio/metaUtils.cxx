                          ...
                          )
{
  ...
  while(seekpos < uncompressedSeekPosition+uncompressedDataSize)
    ...
    unsigned char* outdata = new unsigned char[static_cast<unsigned int>(buffersize)];
    ...
    unsigned char* inputBuffer = new unsigned char[static_cast<unsigned int>(inputBufferSize)];
...
unsigned char * MET_PerformCompression(const unsigned char * source,
  ...
  unsigned char * output_buffer   = new unsigned char[static_cast<unsigned int>(buffer_size)];

  compressedData                  = new unsigned char[static_cast<unsigned int>(buffer_size)];
    ...
    if ( z.avail_in == 0 )
      ...
      if ( count )
        ...
        if(j+count>=buffer_size)
          {
          unsigned char* compressedDataTemp = new unsigned char[static_cast<unsigned int>(j+count+1)];

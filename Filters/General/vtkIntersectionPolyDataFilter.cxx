...
public:
  ...
  static int FindTriangleIntersections(
...
protected:
  ...
  vtkCellArray* SplitCell(vtkPolyData* input, vtkIdType cellId, const vtkIdType* cellPts,
    IntersectionMapType* map, vtkPolyData* interLines, int inputIndex, int numCurrCells);
  ...
  void AddToNewCellMap(
    int inputIndex, int interPtCount, int interPts[3], vtkPolyData* interLines, int numCurrCells);
  ...
  void SetLoopOrientation(vtkPolyData* pd, simPolygon* loop, vtkIdType* nextCell, vtkIdType nextPt,
    vtkIdType prevPt, vtkIdList* pointCells);

  // Get the loop orientation is already given
  int GetLoopOrientation(vtkPolyData* pd, vtkIdType cell, vtkIdType ptId1, vtkIdType ptId2);

  // Orient the triangle based on the transform for remeshing
  void Orient(
    vtkPolyData* pd, vtkTransform* transform, vtkPolyData* boundary, vtkPolygon* boundarypoly);
...
//------------------------------------------------------------------------------
int vtkIntersectionPolyDataFilter::Impl ::FindTriangleIntersections(
  vtkOBBNode* node0, vtkOBBNode* node1, vtkMatrix4x4* transform, void* arg)
{
  ...
  for (vtkIdType id0 = 0; id0 < numCells0; id0++)
  {
    ...
    if (type0 == VTK_TRIANGLE)
    {
      ...
      if (obbTree1->TriangleIntersectsNode(node1, triPts0[0], triPts0[1], triPts0[2], transform))
      {
        ...
        for (vtkIdType id1 = 0; id1 < numCells1; id1++)
        {
          ...
          if (type1 == VTK_TRIANGLE)
          {
            ...
            double surfaceid[2];
            int intersects = vtkIntersectionPolyDataFilter::TriangleTriangleIntersection(triPts0[0],
              triPts0[1], triPts0[2], triPts1[0], triPts1[1], triPts1[2], coplanar, outpt0, outpt1,
              surfaceid, tolerance);
  ...
  int inputIndex, vtkPolyData* output, vtkPolyData* intersectionLines)
{
  ...
  //
  vtkSmartPointer<vtkPolyData> splitLines = vtkSmartPointer<vtkPolyData>::New();
  ...
  for (vtkIdType id = 0; id < splitLines->GetNumberOfPoints(); id++)
  {
    ...
    outPD->InterpolatePoint(input->GetPointData(), newPtId, cell->PointIds, weights);
  ...
  if (input->GetPolys()->GetNumberOfCells() > 0)
  {
    ...
    vtkSmartPointer<vtkIdList> cellsToCheck = vtkSmartPointer<vtkIdList>::New();
...
vtkCellArray* vtkIntersectionPolyDataFilter::Impl ::SplitCell(vtkPolyData* input, vtkIdType cellId,
  const vtkIdType* cellPts, IntersectionMapType* map, vtkPolyData* interLines, int inputIndex,
  int numCurrCells)
{
  // Copy down the SurfaceID array that tells which surface the point belongs
  // to
  vtkIdTypeArray* surfaceMapper;
  surfaceMapper = vtkIdTypeArray::SafeDownCast(interLines->GetPointData()->GetArray("SurfaceID"));

  // Array to keep track of which points are on the boundary of the cell
  vtkSmartPointer<vtkIdTypeArray> cellBoundaryPt = vtkSmartPointer<vtkIdTypeArray>::New();
  ...
  vtkSmartPointer<vtkPoints> points = vtkSmartPointer<vtkPoints>::New();
  vtkSmartPointer<vtkPointLocator> merger = vtkSmartPointer<vtkPointLocator>::New();
  ...
  vtkSmartPointer<vtkCellArray> interceptlines = vtkSmartPointer<vtkCellArray>::New();
  ...
  while (iterLower != iterUpper)
  {
    ...
    interLines->GetLines()->GetCellAtId(lineId, nLinePts, linePtIds);
    ...
    {
      std::map<vtkIdType, vtkIdType>::iterator location = ptIdMap.find(linePtIds[i]);
  ...
  vtkSmartPointer<vtkIdList> nbrCellIds = vtkSmartPointer<vtkIdList>::New();
  ...
  for (vtkIdType ptId = 0; ptId < points->GetNumberOfPoints(); ptId++)
  {
    ...
    if (cellBoundaryPt->GetValue(ptId))
    {
      ...
      angleList->InsertNextValue(atan2(vtkMath::Dot(d, v0), vtkMath::Dot(d, v1)));
      if (ptId > 2)
      {
        // Intersection Point!
  ...
  vtkSmartPointer<vtkPolyData> checkPD = vtkSmartPointer<vtkPolyData>::New();
  ...
  {
    int unique =
      this->CheckLine(checkPD, edgePtIdList->GetValue(id), edgePtIdList->GetValue(id + 1));
    if (unique)
    {
      lines->InsertNextCell(2);
  ...
  int unique = this->CheckLine(checkPD,
    edgePtIdList->GetValue(edgePtIdList->GetNumberOfTuples() - 1), edgePtIdList->GetValue(0));
  if (unique)
  {
    lines->InsertNextCell(2);
    lines->InsertCellPoint(edgePtIdList->GetValue(edgePtIdList->GetNumberOfTuples() - 1));
  ...
  if (interPtIdList->GetNumberOfTuples() > 0 && interceptlines->GetNumberOfCells() > 0)
  {
    ...
    for (int k = 0; k < (int)loops.size(); k++)
    {
      ...
      if (success)
      {
        ...
        {
          vtkIdType t[3] = { idList->GetId(i), idList->GetId(i + 1), idList->GetId(i + 2) };
      ...
      else
      {
        ...
        {
          int numoffsets = 1;
          while ((polys->GetNumberOfCells() != newpd->GetNumberOfPoints() - 2) && numoffsets < 20)
          {
            vtkSmartPointer<vtkDelaunay2D> del2Doffset = vtkSmartPointer<vtkDelaunay2D>::New();
      ...
      for (polys->InitTraversal(); polys->GetNextCell(npts, ptIds);)
      {
        ...
        if (interPtCount >= 2) // If there are more than two, inter line
        {
          ...
          this->AddToNewCellMap(inputIndex, interPtCount, interPts, interLines, numCurrCells);
        }
        numCurrCells++;
      }
      delete[] pointMapper;
  ...
  else // Not (intersection lines and new points)
  {
    ...
    for (polys->InitTraversal(); polys->GetNextCell(npts, ptIds);)
    {
      if (ptIds[0] >= points->GetNumberOfPoints() || ptIds[1] >= points->GetNumberOfPoints() ||
...
//------------------------------------------------------------------------------
int vtkIntersectionPolyDataFilter::Impl ::AddToPointEdgeMap(int index, vtkIdType ptId, double x[3],
  vtkPolyData* mesh, vtkIdType cellId, vtkIdType edgeId, vtkIdType lineId,
  const vtkIdType triPtIds[3])
{
  ...
  PointEdgeMapIteratorType iterLower = this->PointEdgeMap[index]->lower_bound(ptId);
  PointEdgeMapIteratorType iterUpper = this->PointEdgeMap[index]->upper_bound(ptId);
  ...
  int inputIndex, int interPtCount, int interPts[3], vtkPolyData* interLines, int numCurrCells)
{
    ...
    if (temp->GetNumberOfIds() > 0)
    {
      ...
      for (int j = 0; j < temp->GetNumberOfIds(); j++)
      {
        ...
        if (this->NewCellIds[inputIndex]->GetComponent(temp->GetId(j), 0) == -1)
        {
          ...
          this->NewCellIds[inputIndex]->InsertComponent(temp->GetId(j), 0, numCurrCells);
        }
        else
        {
          // Add to new cell mapping data array on intersection lines
          this->NewCellIds[inputIndex]->InsertComponent(temp->GetId(j), 1, numCurrCells);
        }
      }
    }
  }
  ...
  if (interPtCount > 2)
  {
    ...
    if (cellIds[0]->GetNumberOfIds() > 0)
    {
      ...
      {
        if (this->NewCellIds[inputIndex]->GetComponent(cellIds[0]->GetId(j), 0) == -1)
        {
          // Add to new cell mapping data array on intersection lines
          this->NewCellIds[inputIndex]->InsertComponent(cellIds[0]->GetId(j), 0, numCurrCells);
        }
        else
        {
          // Add to new cell mapping data array on intersection lines
          this->NewCellIds[inputIndex]->InsertComponent(cellIds[0]->GetId(j), 1, numCurrCells);
        }
      }
    }
  }
...
int vtkIntersectionPolyDataFilter::Impl ::GetLoops(vtkPolyData* pd, std::vector<simPolygon>* loops)
{
...
int vtkIntersectionPolyDataFilter::Impl ::GetSingleLoop(vtkPolyData* pd, simPolygon* loop,
  vtkIdType nextCell, std::vector<bool>& interPtBool, std::vector<bool>& lineBool)
{
  int intertype = 0;
  vtkSmartPointer<vtkIdList> pointCells = vtkSmartPointer<vtkIdList>::New();
  vtkSmartPointer<vtkIdList> cellPoints = vtkSmartPointer<vtkIdList>::New();
  ...
  while (nextPt != startPt)
  {
    ...
    {
      vtkSmartPointer<vtkPolyData> currentpd = vtkSmartPointer<vtkPolyData>::New();
      vtkSmartPointer<vtkCellArray> currentcells = vtkSmartPointer<vtkCellArray>::New();
...
int vtkIntersectionPolyDataFilter::Impl ::FollowLoopOrientation(vtkPolyData* pd, simPolygon* loop,
  vtkIdType* nextCell, vtkIdType nextPt, vtkIdType prevPt, vtkIdList* pointCells)
{
  ...
  for (vtkIdType i = 0; i < pointCells->GetNumberOfIds(); i++)
  {
    ...
    if (*nextCell != cellId)
    {
      ...
      if (neworient == loop->orientation)
      {
        ...
        vtkSmartPointer<vtkIdList> specialCellPoints = vtkSmartPointer<vtkIdList>::New();
...
void vtkIntersectionPolyDataFilter::Impl ::SetLoopOrientation(vtkPolyData* pd, simPolygon* loop,
  vtkIdType* nextCell, vtkIdType nextPt, vtkIdType prevPt, vtkIdList* pointCells)
{
  ...
  vtkPolyData* pd, vtkIdType cell, vtkIdType ptId1, vtkIdType ptId2)
{
  ...
  if (fabs(area) < 1e-10)
  {
    ...
    vtkDebugWithObjectMacro(
      this->ParentFilter, << "Double check area with more accurate transform");
    vtkSmartPointer<vtkPoints> testPoints = vtkSmartPointer<vtkPoints>::New();
    vtkSmartPointer<vtkPolyData> testPD = vtkSmartPointer<vtkPolyData>::New();
    vtkSmartPointer<vtkCellArray> testCells = vtkSmartPointer<vtkCellArray>::New();
...
void vtkIntersectionPolyDataFilter::Impl ::Orient(
  vtkPolyData* pd, vtkTransform* transform, vtkPolyData* boundary, vtkPolygon* boundarypoly)
{
...
int vtkIntersectionPolyDataFilter::Impl::GetTransform(vtkTransform* transform, vtkPoints* points)
{
...
//------------------------------------------------------------------------------
int vtkIntersectionPolyDataFilter::TriangleTriangleIntersection(double p1[3], double q1[3],
  double r1[3], double p2[3], double q2[3], double r2[3], int& coplanar, double pt1[3],
  double pt2[3], double surfaceid[2], double tolerance)
{
  double n1[3], n2[3];
...
void vtkIntersectionPolyDataFilter::CleanAndCheckSurface(
...
void vtkIntersectionPolyDataFilter::CleanAndCheckInput(vtkPolyData* pd, double tolerance)
{
  vtkSmartPointer<vtkCleanPolyData> cleaner = vtkSmartPointer<vtkCleanPolyData>::New();
  vtkSmartPointer<vtkTriangleFilter> triangulator = vtkSmartPointer<vtkTriangleFilter>::New();
  ...
  vtkIdType npts;
  double pt0[3], pt1[3], pt2[3];
  normaler->GetOutput()->GetPolys()->GetCellAtId(0, npts, cellPts);
  ...
  normaler->GetOutput()->GetCellData()->GetNormals("Normals")->GetTuple(0, arrayNormal);
}
  ...
  vtkInformationVector** inputVector, vtkInformationVector* outputVector)
{
  ...
  if (input0 == nullptr || input1 == nullptr || input0->GetNumberOfPoints() == 0 ||
    input1->GetNumberOfPoints() == 0 || input0->GetNumberOfCells() == 0 ||
  ...
  vtkPolyData* outputIntersection =
    vtkPolyData::SafeDownCast(outIntersectionInfo->Get(vtkDataObject::DATA_OBJECT()));
  ...
  vtkPolyData* outputPolyData0 =
    vtkPolyData::SafeDownCast(outPolyDataInfo0->Get(vtkDataObject::DATA_OBJECT()));

  vtkPolyData* outputPolyData1 =
    vtkPolyData::SafeDownCast(outPolyDataInfo1->Get(vtkDataObject::DATA_OBJECT()));
  ...
  vtkSmartPointer<vtkCleanPolyData> lineCleaner = vtkSmartPointer<vtkCleanPolyData>::New();
...
//------------------------------------------------------------------------------
int vtkIntersectionPolyDataFilter::FillInputPortInformation(int port, vtkInformation* info)
{

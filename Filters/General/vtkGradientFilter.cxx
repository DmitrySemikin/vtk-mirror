...
int GetCellParametricData(vtkIdType pointId, double pointCoord[3], vtkCell* cell,
                          int& subId, double parametricCoord[3]);

// Functions for image data and structured grids
template <class GridT, class DataT>
void ComputeGradientsSG(GridT* output, DataT* array, DataT* gradients,
  ...
  vtkInformationVector** inputVector, vtkInformationVector* outputVector)
{
  ...
  {
    this->ComputeRegularGridGradient(array, fieldAssociation, computeVorticity,
                                     computeQCriterion, computeDivergence, output);
  }
  else
  {
    this->ComputeUnstructuredGridGradient(array, fieldAssociation, input,
                                          computeVorticity, computeQCriterion,
...
namespace //begin anonymous namespace
{
  ...
  struct GradientsBase
  {
    ...
    GradientsBase(TData *a, int numComp, TData *g,
                  TData *v, TData *q, TData *d) :
      Array(a), NumComp(numComp), Gradients(g), Vorticity(v),
      QCriterion(q), Divergence(d) {}
  };
  ...
  struct PointGradients : public GradientsBase<TData>
  {
    ...
    vtkSMPThreadLocal<vtkSmartPointer<vtkIdList>> CurrentPoint;
    vtkSMPThreadLocal<vtkSmartPointer<vtkIdList>> CellsOnPoint;
    ...
    void Initialize()
    {
      ...
      this->CurrentPoint.Local().TakeReference(vtkIdList::New());
      this->CurrentPoint.Local()->Allocate(24);
      this->CurrentPoint.Local()->SetNumberOfIds(1);
      this->CellsOnPoint.Local().TakeReference(vtkIdList::New());
    }
    ...
    void operator()(vtkIdType ptId, vtkIdType endPtId)
    {
      ...
      auto& currentPoint = this->CurrentPoint.Local();
      ...
      for ( ; ptId < endPtId; ptId++)
      {
        ...
        if ( this->CellOption == vtkGradientFilter::Patch )
        {
          ...
          {
            input->GetCell(cellsOnPoint->GetId(neighbor),cell);
  ...
  struct PointGradientsWorker
  {
    ...
    void operator()(DataT* array, vtkDataSet *input,
                    vtkDataArray *gradients, vtkDataArray *vorticity,
                    vtkDataArray *qCriterion, vtkDataArray *divergence,
                    int highestDim, int cellOption)
    {
      vtkIdType numPts = input->GetNumberOfPoints();
      int numComp = array->GetNumberOfComponents();

      PointGradients<DataT> pg(input, array, numComp, (DataT*)gradients,
                               (DataT*)vorticity, (DataT*)qCriterion,
                               (DataT*)divergence, highestDim, cellOption);
  ...
  struct CellGradientsWorker
  {
                    ...
                    vtkDataArray *qCriterion, vtkDataArray *divergence)
    {
      ...
      CellGradients<DataT> cg(input, array, numComp, (DataT*)gradients,
                              (DataT*)vorticity, (DataT*)qCriterion,
                              (DataT*)divergence);
  ...
  {
    template <class DataT>
    void operator()(DataT *array, vtkDataSet *output,
                    vtkDataArray *gradients, vtkDataArray *vorticity,
                    vtkDataArray *qCriterion, vtkDataArray *divergence,
                    int fieldAssociation)
    {
...
//------------------------------------------------------------------------------
int vtkGradientFilter::
ComputeUnstructuredGridGradient(vtkDataArray* array, int fieldAssociation,
                                vtkDataSet* input, bool computeVorticity,
                                bool computeQCriterion, bool computeDivergence,
                                vtkDataSet* output)
{
  ...
  if (fieldAssociation == vtkDataObject::FIELD_ASSOCIATION_POINTS)
  {
    if (!this->FasterApproximation)
    {
      ...
      else if (vtkUnstructuredGrid* ug = vtkUnstructuredGrid::SafeDownCast(input))
      {
        ug->BuildLinks();
      }
      ...
      if ( !PointGradientsDispatch::Execute(array, pgWorker, input, gradients,
                                            vorticity, qCriterion, divergence,
                                            highestCellDimension, this->ContributingCellOption) )
      {
        pgWorker(array, input, gradients, vorticity, qCriterion, divergence,
                 highestCellDimension, this->ContributingCellOption);
      }
    ...
    else // this->FasterApproximation
    {
      ...
      if ( !CellGradientsDispatch::Execute(array, cgWorker, output, cellGradients,
                                           cellVorticity, cellQCriterion, cellDivergence) )
      {
  ...
  else // fieldAssociation == vtkDataObject::FIELD_ASSOCIATION_CELLS
  {
    ...
    using CellGradientsDispatch = vtkArrayDispatch::DispatchByValueType<vtkArrayDispatch::Reals>;
    CellGradientsWorker cgWorker;
    if ( !CellGradientsDispatch::Execute(pointScalars, cgWorker, input, gradients,
                                         vorticity, qCriterion, divergence) )
    {
...
//------------------------------------------------------------------------------
int vtkGradientFilter::
ComputeRegularGridGradient(vtkDataArray* array, int fieldAssociation,
                           bool computeVorticity, bool computeQCriterion,
                           bool computeDivergence, vtkDataSet* output)
{
  int arrayType = this->GetOutputArrayType(array);
  int numberOfInputComponents = array->GetNumberOfComponents();
  vtkSmartPointer<vtkDataArray> gradients = nullptr;
  ...
  if ( !StructuredGradientsDispatch::Execute(array, sgWorker, output, gradients,
                                             vorticity, qCriterion, divergence, fieldAssociation) )
  {
    sgWorker(array, output, gradients, vorticity, qCriterion, divergence,
             fieldAssociation);
  }
...
//------------------------------------------------------------------------------
// Process structured dataset types. Currently this is a serial operation,
                        ...
                        DataT* vorticity, DataT* qCriterion, DataT* divergence)
{
  ...
  std::vector<double> localGradients(numberOfInputComponents * 3);
  ...
  for (int k = 0; k < dims[2]; k++)
  {
    for (int j = 0; j < dims[1]; j++)
    {
      for (int i = 0; i < dims[0]; i++)
      {
        ...
        if (gradients)
        {
          auto grad = vtk::DataArrayTupleRange(gradients);
          auto g = grad[idx];
        ...
        if (vorticity)
        {
          auto vort = vtk::DataArrayTupleRange(vorticity);
          ComputeVorticityFromGradient(&localGradients[0], vort[idx]);
        }
        if (qCriterion)
        {
          auto qCrit = vtk::DataArrayTupleRange(qCriterion);
          ComputeQCriterionFromGradient(&localGradients[0], qCrit[idx]);
        }
        if (divergence)
        {
          auto div = vtk::DataArrayTupleRange(divergence);

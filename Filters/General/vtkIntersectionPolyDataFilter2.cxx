/*=========================================================================
  ...
  Module:    vtkIntersectionPolyDataFilter2.cxx
...
::SplitMesh(int inputIndex,vtkPolyData *output,vtkPolyData *intersectionLines)
{
  ...
  if ( input->GetPolys()->GetNumberOfCells() > 0 )
    ...
    for (cells->InitTraversal(); cells->GetNextCell(nptsX, pts); cellIdX++)
      ...
      if ( nptsX != 3 )
        {
        vtkGenericWarningMacro( << "vtkIntersectionPolyDataFilter2 only works"
     ...
     // // Renumber the point IDs.
      ...
      for (polys->InitTraversal(); polys->GetNextCell(npts, ptIds); )
        ...
        if (interPtCount >= 2) //If there are more than two, inter line
          ...
          //This is used in vtkBooleanOperationPolyDataFilter2
...
//----------------------------------------------------------------------------
int vtkIntersectionPolyDataFilter2::TriangleTriangleIntersection(
...
void vtkIntersectionPolyDataFilter2::CleanAndCheckSurface(vtkPolyData *pd,
...
//----------------------------------------------------------------------------
int vtkIntersectionPolyDataFilter2::RequestData(
                                        ...
                                        vtkInformationVector*  outputVector)
{
  ...
  vtkSmartPointer< vtkPointLocator > pointMerger =
    vtkSmartPointer< vtkPointLocator >::New();
  pointMerger->SetTolerance(sqrt(2)*this->Tolerance);

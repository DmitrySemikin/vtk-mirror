...
int vtkWarpLens::FillInputPortInformation(int vtkNotUsed(port), vtkInformation* info)
{
  ...
  vtkInformationVector** inputVector, vtkInformationVector* outputVector)
{
  ...
  for (ptId = 0; ptId < numPts; ptId++)
  {
    ...
    newPixel[1] = (newY - this->PrincipalPoint[1]) / this->FormatHeight * this->ImageHeight * -1;
...
void vtkWarpLens::PrintSelf(ostream& os, vtkIndent indent)
{
  ...
  os << indent << "PrincipalPoint: (" << this->PrincipalPoint[0] << ", " << this->PrincipalPoint[1]
     << ") in mm\n";

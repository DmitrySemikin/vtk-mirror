...
void vtkCookieCutter::SetLoopsConnection(vtkAlgorithmOutput* output)
{
  ...
  const double* segP1, const double* segP2, const double* a1, const double* a2, double px[3])
{
  ...
  if (std::fabs(1.0 - u) <= 10.0 * VTK_DBL_EPSILON || std::fabs(1.0 - v) <= 10.0 * VTK_DBL_EPSILON)
    return false;
...
void vtkCookieCutter::CreateDefaultLocators()
{
  if (!this->CellLocator)
  {

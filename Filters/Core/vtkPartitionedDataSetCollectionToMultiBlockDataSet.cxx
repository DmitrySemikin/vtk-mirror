...
public:
  ...
  void Visit(int nodeid) override
  {
    ...
    auto pd = this->Input->GetPartitionedDataSet(dataset_indices[0]);
    if (pd->GetNumberOfPartitions() == 1)
    {
      this->AddToBack(pd->GetPartition(0), this->GetCurrentNodeName());
    }
    else
    {
  ...
  void EndSubTree(int nodeid) override { this->Stack.pop_back(); }

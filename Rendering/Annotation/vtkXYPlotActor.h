...
public:
	...
	void RemoveDataSetInput(vtkDataSet *ds, const char* arrayName,
			int component);
	...
	void SetAdjustXLabels(int adjust);vtkGetMacro( AdjustXLabels, int )
	;
	void SetAdjustYLabels(int adjust);vtkGetMacro( AdjustYLabels, int )
	;
	...
	virtual void SetTitleTextProperty(vtkTextProperty *p);vtkGetObjectMacro(TitleTextProperty,vtkTextProperty)
	;
	...
	virtual void SetAxisTitleTextProperty(vtkTextProperty *p);vtkGetObjectMacro(AxisTitleTextProperty,vtkTextProperty)
	;

	// Description:
	// Set/Get the labels text property of all axes. Note that each axis can
	// be controlled individually through the GetX/YAxisActor2D() methods.
	virtual void SetAxisLabelTextProperty(vtkTextProperty *p);vtkGetObjectMacro(AxisLabelTextProperty,vtkTextProperty)
	;
	...
	virtual void SetXLabelFormat(const char*);vtkGetStringMacro(XLabelFormat)
	;

	// Description:
	// Set/Get the format with which to print the Y label.
	virtual void SetYLabelFormat(const char*);vtkGetStringMacro(YLabelFormat)
	;
	...
	void ViewportToPlotCoordinate(vtkViewport *viewport);vtkSetVector2Macro(PlotCoordinate,double)
	...
	// can be wrapped.)
	void PlotToViewportCoordinate(vtkViewport *viewport);vtkSetVector2Macro(ViewportCoordinate,double)
...
protected:
	...
	virtual void CreatePlotData(int *pos, int *pos2, double xRange[2],
			double yRange[2], double *norms, int numDS, int numDO);
	...
	double *TransformPoint(int pos[2], int pos2[2], double x[3],
			double xNew[3]);
...
private:
	...
	int IsInputPresent(vtkAlgorithmOutput* in, const char* arrayName,
			int component);
